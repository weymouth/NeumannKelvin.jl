...
Fn,B=0.2,0.125; ps = (ϕ=∫kelvin₂,ℓ=Fn^2)
...
for (n,c) in zip((20,40,60),colorschemes[:Blues_4][2:end])
    ...
    plot!(x,x->ζ(x,0,q,panels;ps...)*ps.ℓ/B,label="h/L=1/$n",
        line=(2,:dash),marker=(2,),markerstrokewidth=0;c)
end;plot!(xlabel="x/L",ylabel="ζ/B",title="Thin-Ship Wigley B=L/8, Fn=$Fn")
savefig("examples/thinWigleyWL.png")

begin
    h = 1/40; panels = wigley(h;B)
    q = components(panels.n,1)/4π;
    contour(-2:h:1,-1:0.5h:1,(x,y)->ζ(x,y,q,panels;ps...)*ps.ℓ/B,
...
data = map(Fn->(Fn=Fn,Cw=Cw(panels;ps...,ℓ=Fn^2)/0.5B^2),logrange(0.2,1.2,120)) |> Table
