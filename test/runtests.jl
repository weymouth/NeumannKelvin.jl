...
@testset "panels.jl" begin
    ...
    for c in [1,0.05,0.01]
        u = NeumannKelvin.arcsplit(circ,0,2pi,4,c)
        @test u.mid ≈ pi
        @test u.len ≈ max(4,sqrt(2/c))*2pi rtol=0.05
    end
    ...
    @test length(surfsplit(torus,0.,2pi,0.,2pi;depthmax=2))==4 # 2^depthmax
    uvs = surfsplit(spheroid,0.,pi,0.,2pi,c=Inf) |> Table
    @test count(first.(uvs.low) .== 0 )==count(first.(uvs.high) .≈ pi )==4
    @test length(uvs)>16 # found the belly!

    panelize(spheroid,0,pi,0,2pi,hᵤ=0.6,c=Inf)
...
@testset "panel_method.jl" begin
    ...
    panels = measurepanel.(S,[π/4,3π/4]',π/4:π/2:2π,π/2,π/2,cubature=true) |> Table
    @test size(panels) == (8,)
