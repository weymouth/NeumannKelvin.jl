...
∫kelvin₂(x,p;χ=true,kwargs...) = ∫kelvin(x,p;χ,kwargs...)+∫kelvin(x,reflect(p,2);χ,kwargs...)
...
for (n,c) in zip((32,48,64,80),colorschemes[:Blues_4][1:end])
    ...
    q = influence(panels;ϕ=∫kelvin₂,Fn)\components(panels.n,1)
    x,y,_ = filter(onwaterline,panels) |> p -> components(p.x)
    z = map(xy->2ζ(xy[1],xy[2],q,panels;ϕ=∫kelvin₂,Fn),zip(x,y))
...
for (Fn,c) in zip((0.2,0.316,0.4),colorschemes[:Blues_3])
    q = influence(panels;ϕ=∫kelvin₂,Fn,filter=false)\components(panels.n,1)
    z = map(xy->2ζ(xy[1],xy[2],q,panels;ϕ=∫kelvin₂,Fn,filter=false),zip(x,y))
