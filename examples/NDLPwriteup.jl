...
For a smooth curve $r(u)$, the leading-order normal deviation $\delta$ of the curve from its chord over a small arclength interval $\Delta l$ is governed by the curvature $\kappa$:

$\delta \approx \frac{\Delta l^2\kappa}{8}$

NDLP exploits this fact directly, and constructs a parameterization $s(u)$ whose local metric is chosen so that the maximum normal deviation per segment is bounded a priori:

$\delta \leq d_n \Delta s$

where $\Delta s$ is the maximum segment length and $d_n$ is a specified normal deviation tolerance. Because the bound is embedded directly into the parameterization, segments automatically satisfy the error limit without iteration or refinement.
...
Consider a curve $r(u), u ∈ [u₀,u₁]$ with prescribed maximum allowed curve-segment length $\Delta l \le \Delta s$ and maximum allowed normal deviation of the straight-line segment from the curve $\delta\le d_n\Delta s$, where $\Delta s$ is the prescribed length limit and $d_n$ is the prescribed relative normal deviation tolerance.

We begin by defining three segmentation methods: 1. adaptive subdivision, 2. curvature-weighted sampling, and 3. the new NDLP sampling.
...
This notebook shows the NDLP method achieves tight bounds to a deviation limit by construction, while being no more complex to implement that standard curve sampling approaches. The fact that the method requires no iteration or tuning, while being robust to small changes in the curve shape and sampling tolerances makes it appealing for use in many downstream applications.
"""
