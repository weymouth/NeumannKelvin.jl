...
\subsection*{5.4 Wave Resistance}
\label{sec:wave_resistance}

The wave resistance follows from the Havelock formula, which expresses $R_w$ in terms of the Kochin function $K(t)$ — the far-field amplitude of the radiated wave pattern. Working directly in the variable $t = \sin\theta/\cos^2\theta$ with Jacobian $d\theta/\cos^3\theta = (1+t^2)/(1+2t^2)\,dt$, the Havelock formula becomes
%
\begin{equation}
...
\end{equation}
%
The Kochin function is the source-strength-weighted integral of the wavelike kernel over the planform. For the elliptic spanwise distribution $q_0\sqrt{1-(y/b)^2}$ concentrated on the leading edge at $x=0$ and trailing edge at $x=L$, the Kochin function is
%
\begin{equation}
  K(t) = q_0\ell^2 \int_{-b}^{b} \sqrt{1-(y/b)^2}\,
  \bigl[e^{ig(L,y,t)} - e^{ig(0,y,t)}\bigr]\, dy,
  \label{eq:kochin}
\end{equation}
%
where $g(x,y,t) = (x+yt)\sqrt{1+t^2}$ is the wavelike phase from Section~\ref{sec:unified}. The streamwise interference factor pulls out immediately
%
\begin{equation}
  K(t) = q_0\ell^2\,\bigl(e^{iL\sqrt{1+t^2}} - 1\bigr)
  \int_{-b}^{b} \sqrt{1-(y/b)^2}\, e^{iyk(t)}\, dy
  = \pi q_0\ell^2\, \frac{J_1(bk(t))}{k(t)}\,\bigl(e^{iL\sqrt{1+t^2}} - 1\bigr),
  \label{eq:kochin_factored}
\end{equation}
%
where $k(t) = t\sqrt{1+t^2}$ and the spanwise integral is the Fourier transform of the elliptic distribution. The interference phase $L\sqrt{1+t^2} = g(L,0,t)$ is the wavelike phase at $y=0$, with stationary point at $t=0$ corresponding to transverse waves propagating directly behind the ship.

Substituting~\eqref{eq:kochin_factored} into~\eqref{eq:havelock} and defining the wave drag coefficient $C_W = R_w/(\rho U^2 b^2\ell^2)$ gives
%
\begin{equation}
...
Note that we can use the same approach for the wave resistance integral~\eqref{eq:Cw} by defining $A_w(t)=(J_1(bk(t))/bk(t))^2 (1+t^2)/(1+2t^2)$ and using the same Hankel function decomposition for contour deformation. The fast $t^{-3}$ decay of $A_w$ means direct quadrature is also practical.
