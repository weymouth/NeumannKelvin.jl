...
function ∫Pwave(a::SVector{2},b::SVector{2},c::SMatrix{M,N};z=-0.,ltol=-5log(10),atol=10exp(ltol)) where {M,N}
    ...
    Δx,x₀ = b-a,(a+b)/2
    j(m,x) = sign(x)^m*sphericalbesselj(m,abs(x))
    jm(t) = SVector{M}([im^m*j(m,Δx[1]*kₓ(t)/2) for m in 0:M-1])
    jn(t) = SVector{N}([im^n*j(n,Δx[2]*t*kₓ(t)/2) for n in 0:N-1])
    ...
    I₀ + prod(Δx)/4*sum(Iterators.product(1:2,1:2)) do (k₁,k₂)
        γ(t) = transpose(Am((-1)^k₁,t))*c*An((-1)^k₂,t)
        ∫Wᵢ(x[k₁],y[k₂],z,rngs[k₁,k₂]\rngs₀;γ,atol)
    end
end
x,y=-√8,-2.;∫Pwave(SA[x-1,y-1],SA[x+1,y+1],SA[0 1]')
using Plots
plot!(range(-2,0,100),y->∫Pwave(SA[x-1,y-1],SA[x+1,y+1],SA[0 1]'))
