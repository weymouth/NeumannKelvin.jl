...
using NeumannKelvin:kelvin,nearfield,wavelike,quadgl
...
function ∫₂wavelike(x,z,a,b,ltol=-5log(10))
    ...
    I(y;f) = 4complex_path(t->g(x,y,t)-im*z*(1+t^2),
        t->dg(x,y,t)-2im*z*t,rngs,γ=t->-im/k(t);f)
    I(b;f)-I(a,f=zero)
end
k(t) = t*⎷(1+t^2)

∫₂wavelike(-3.6,-0.,1/8-0.01,1/8+0.01)

check(x,z,ϵ) = (
    @show ∫₂wavelike(x,z,-ϵ,ϵ);
    @show 2ϵ*wavelike(x,0.,z))
check(-1.,-0.01,0.02)

using Plots
plot(range(-5,-0,1000),x->∫₂wavelike(x,-0.,-0.5,0.5))
plot(range(-5,-0,1000),x->wavelike(x,-0.1,-0.))
