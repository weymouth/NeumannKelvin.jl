...
function ∫contour(ξ,p;Fn,z_max=-1/50Fn^2)
    ...
    n₁dx₂ = -dx[2]^2/norm(dx)
    G = 0.5sum(α->kelvin(ξ,reflect(p.x,3)+α*dx;Fn,z_max),(-0.5/√3,0.5/√3))
    Fn^2*G*n₁dx₂
end
function ∫surface(x,p;χ=true,kwargs...)
    (!χ || !onwaterline(p)) && return ∫kelvin(x,p;kwargs...) # no waterline
    ∫kelvin(x,p;kwargs...)+∫contour(x,p;kwargs...)
end
...
for (n,c) in zip((32,48,64,80),colorschemes[:Blues_4][1:end])
    h = 1/n; panels = wigley(h;B,D); z_max = -h/Fn^2
    q = influence(panels;ϕ=∫surface₂,Fn,z_max)\components(panels.n,1)
    x,y,_ = filter(onwaterline,panels) |> p -> components(p.x)
    z = map(xy->2ζ(xy[1],xy[2],q,panels;ϕ=∫surface₂,Fn,z_max),zip(x,y))
...
for (Fn,c) in zip((0.2,0.316,0.4),colorschemes[:Blues_3])
    q = influence(panels;ϕ=∫surface₂,Fn,z_max=-0.)\components(panels.n,1)
    z = map(xy->2ζ(xy[1],xy[2],q,panels;ϕ=∫surface₂,Fn,z_max=-0.),zip(x,y))
